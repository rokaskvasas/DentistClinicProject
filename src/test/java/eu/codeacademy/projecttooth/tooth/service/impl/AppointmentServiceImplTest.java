...
class AppointmentServiceImplTest {
    ...
    void GetAppointmentPageable() {
        when(appointmentRepository.findAllByPatientUserUserId(anyLong(),any(PageRequest.class))).thenReturn(new PageImpl<>(new ArrayList<>()));
        assertTrue(appointmentServiceImpl.getAppointmentPageable(123L, 10, 3).toList().isEmpty());
        verify(appointmentRepository).findAllByPatientUserUserId(anyLong(),any(PageRequest.class));
        assertTrue(appointmentServiceImpl.getAppointmentList().isEmpty());
    }

    @Test
    void GetAppointmentPageableThrowsException() {
        when(appointmentRepository.findAllByPatientUserUserId(anyLong(),any(PageRequest.class))).thenThrow(new ObjectNotFoundException("Msg"));
